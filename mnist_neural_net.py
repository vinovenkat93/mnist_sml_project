...
def cross_validation_k_fold(alpha = 'default', act_fcn = 'default', learn_rate = 'default'):
    ...
    for k in xrange(10):
        ...
        if alpha == 'default': alpha = 1e-4
        if learn_rate == 'default': learn_rate = 1e-3
        if act_fcn == 'default': act_fcn = 'relu'
        
        accuracy[k], y_score = neural_net_opt(alpha, act_fcn, learn_rate, train, test)
        
        # Filename based on param being optimized
        if alpha == 'default' and act_fcn == 'default':
        ...
        elif alpha == 'default' and learn_rate == 'default':
